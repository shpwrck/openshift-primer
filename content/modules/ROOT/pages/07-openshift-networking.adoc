...
Services provide a single fully qualified domain name (FQDN) and a single IP on which a set of workloads can all be addressed. This design makes accessing services predicatable and reliable.
...
`Routes` are an ingress implementation and 100% compatible with standard Kubernetes. You could deploy all of the necessary components in any Kubernetes cluster using the https://github.com/openshift/router[github source]. Despite this however, `Routes` are not common outisde of OpenShift configuration. The most common solution for Kubernetes networking is Ingress. 
